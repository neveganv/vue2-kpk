	...
	if (req.body) {
		...
		prepareCourse
			...
			.catch(err => {
				response.status = 500;
				response.message =
					'Some error occurred while creating the preparing course.';
				response.error.type = '';
		...
		response.error.message =
			'Name , surname, middle name , parant phone, student phone , pass id is required';
		res.status(response.status).send(response);
	}
};
...
exports.findAll = async (req, res) => {
	...
	PrepareCourse.find()
		...
		.catch(err => {
			...
			response.error.type = '';
			response.error.message =
				'Some error occurred while find the prepare course.';
			return res.status(response.status).send(response);
		});
};
...
exports.deleteApplications = async (req, res) => {
	...
	try {
		for (let i = 0; i < applications.length; i++) {
			...
			PrepareCourse.find({ _id: applications[i] }.remove().exec());
		}
		...
		// response.error.type = '';
		// response.error.message =
			'Some error occurred while delete the prepare course.';
		return res.status(response.status).send(response);
	}
};
